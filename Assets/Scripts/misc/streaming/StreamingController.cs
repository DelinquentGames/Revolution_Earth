...
public class StreamingController : MonoBehaviour {
    ...
    void Update()
    {
    	...
    	if(!GameManager.menuVisible) {
			...
			if(_mouseControl) {
                camera.transform.localEulerAngles = new Vector3(_hOfs, _vOfs, camera.transform.localEulerAngles.z);				
			}
    	}
            ...
            case StreamingStep.Helicopter:
                ...
                if(currentOp != null)
                {
                    if(currentOp.isDone)
                    {
                        ...
                        Component[] cameras = auxGO.GetComponentsInChildren<Camera>() as Component[];
    ...
    { 	
        ...
        Transform cutsceneAni = helicopterGO.transform.parent.parent;
        ...
        while(time2Shake>0.0) {            
            transform.localEulerAngles = new Vector3(transform.localEulerAngles.x + Random.Range(-1.0f,1.0f) * Mathf.Clamp01(time2Shake), transform.localEulerAngles.y, transform.localEulerAngles.z);
        	
        	yield return time2Shake;
        ...
        {
        	if(readyToLoadTerrain && !GameManager.menuVisible) {
	        ...
	        float time2Play = 3.0f + Random.value * 2.0f;
	        
	        EnableMouseControl(false);
	        cloudBed.SendMessage("SetCut", 1.0);
	        
	        float aniWeight = 1.0f;
        ...
        */
        ...
        AudioListener listener = gameObject.GetComponent("AudioListener") as AudioListener;
