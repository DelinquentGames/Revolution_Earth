...
public class GameQualitySettings : MonoBehaviour {
	...
	public GameObject[] ambientParticleObjects = new GameObject[1];
	private ArrayList  _ambientParticleObjectSettings;
	...
	void AutoChooseQualityLevel ()
	{
		...
		float fillneed = (resx*resy + 400*300) * (30.0f / 1000000.0f);
		float[] levelmult = {5.0f, 30.0f, 80.0f, 130.0f, 200.0f, 320.0f};
		
				
		var level = 0;
		//Debug.Log((QualitySettings.names.Length - 1).ToString() + " : " + QualitySettings.names[QualitySettings.names.Length - 1]);
        while (level < (QualitySettings.names.Length - 1) && fillrate > fillneed * levelmult[level + 1])
			++level;

		overallQuality = level;	
		//NGUIDebug.Log(overallQuality);
		switch(overallQuality){
		case 0:
			QualityValue.value = 0f;
			break;
		case 1:
			QualityValue.value = 0.2f;
			break;
		case 2:
			QualityValue.value = 0.4f;
			break;
		case 3:
			QualityValue.value = 0.6f;
			break;
		case 4:
			QualityValue.value = 0.8f;
			break;
		case 5:
			QualityValue.value = 1f;
			break;
		}
	...
	void UpdateAmbientParticleQuality() 
	{
		...
		{	
			...
			for(int k = 0; k < ambientParticleObjects.Length; k++)
			{
				AmbientParticleSettings setting = _ambientParticleObjectSettings[k] as AmbientParticleSettings;
	...
	void InitializeSceneSettings()
	{
        ...
        else if(Application.loadedLevelName == "TutorialIslsand")
        {
            currentScene = 1;
        }
	...
	void InitializeCameraSettings()
	{
		...
		if(cameras != null)
		{
			...
			{
				float dynamicDistance = (dynamicObjectsFarClip * (dynamicLayersRange.y - dynamicLayersRange.x)) + dynamicLayersRange.x;
				
				for(int d = 0; d < dynamicLayers.Length; d++)
				{
            ...
            BloomAndLensFlares bloomFlares;
            SunShafts shafts;
            DepthOfField34 depth;
			...
			if(cameras.Length > 0)
			{
				for(int c = 0; c < cameras.Length; c++)
				{
                    ...
                    cCorrection = cameras[c].GetComponent("ColorCorrectionCurves") as ColorCorrectionCurves;
                    if(cCorrection != null) cCorrection.enabled = colorCorrection;

                    bloomFlares = cameras[c].GetComponent("BloomAndLensFlares") as BloomAndLensFlares;
                    if(bloomFlares != null) bloomFlares.enabled = bloomAndFlares;

                    shafts = cameras[c].GetComponent("SunShafts") as SunShafts;
                    ...
                    depth = cameras[c].GetComponent("DepthOfField34") as DepthOfField34;
                    if(depth != null ) depth.enabled = depthOfFieldAvailable;

                    screenSpaceAO = cameras[c].GetComponent("SSAOEffect") as SSAOEffect;
	...
	void Update()
	{
		if(GameManager.menuVisible || mainMenu)
		{
			UpdateAllSettings ();
		}
	}
	...
	{
		float dObjectDistance = 50.0f;
		float dObjectDensity = 1.0f;
		float nPError = 5.0f;
		float tDistance = 400.0f;
		int lod = 2;
		float billboards = 70.0f;
		int mTrees = 60;
		float fPError = 5.0f;
	...
	{
		//Debug.Log("Making it to Update Quality Settings!");
