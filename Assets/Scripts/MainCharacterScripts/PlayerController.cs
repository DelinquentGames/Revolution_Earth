...
{
	//This script needs a handle on the camera script
	//for the sensitivity settings.
	public CharacterCamera				cameraScript			= null;

	//A cache of the player transform.
	...
	//This will be updated by this script, and provides the current animation state.
	...
	public static int 	idleState 				= Animator.StringToHash("MoveTree.Idle_Neutral_1");	
	...
	void Start () 
	{
		...
		foreach (Transform t in bones)
		{
			...
			if(t.collider && t.collider != character.collider)
	...
	void OnAnimatorMove ()
	{
		...
		if(baseState == BaseState.Base || baseState == BaseState.Combat)
		{
			...
			character.rotation *= a.deltaRotation; 	
		}
	}
	//=================================================================================================================o
	...
	void FixedUpdate () 
	{
		...
		if (IsEnabled){
			...
			doAtk1Down = Input.GetButtonDown("Fire1");
			doAtk1 = Input.GetButton("Fire1");
			doAim = Input.GetButton("Fire2");
			doAtk2Down = Input.GetButtonDown("Fire2");
			doAtk2 = Input.GetButton("Fire2");
		...
		if(a)
		{
			grounded = Physics.Raycast (character.position + character.up * col.center.y,
			                            character.up * -1, out groundHit, groundedDistance, groundLayers);
	...
	void _Base ()
	{
		...
		currentAnimState 	= st.nameHash;
		
		if(grounded)
		{
//			//Rotate the player about the Y axis , using the ZAXIS for rotation
//			//Right Joystic or Mouse.
//			//The camera script will automatically snap in place behind the player
//			//when the character enters motion.
//			
//			//If the player is moving, always rotate the player around the the world Y axis, 
//			//using the right joystick ZAXIS Horiz movement, or MouseX.
//			if (v != 0.0f && st.nameHash == idleState)
//			{
//				//If the player is transitioning from idle to locomotion, the player must SNAP (NOT SMOOTH) into that same direction,
//				//about the world y axis of the camera.
//				//this will provide a decent control system, specially when something sudden needs to happen, 
//				//and the camera is facing this, as the idle state allows for an orbit cam
//				
//				//Obtain camera world Y axis rotation.
//				//Apply to character.
//				localEuler.x = thisTransform.localEulerAngles.x;
//				localEuler.y = cameraScript.thisTransform.localEulerAngles.y;
//				localEuler.z = thisTransform.localEulerAngles.z;
//				
//				thisTransform.localEulerAngles = localEuler;
//				
//			} //if moving
//
//			//NGUIDebug.Log(st.nameHash.ToString() + " | " + Animator.StringToHash("MoveTree").ToString());
//			//As long as the player is not standing still, 
//			//the ZAXIS Right Joystick Horiz, or Mouse X will 
//			//rotate the player around the world up vector.
//			if (st.nameHash != idleState)
//			{
//				//Obtain Z Axis rotation.
//				//This Script will cater for Mouse and Joystick input, on the ZAxis rotation (Right Joystick).
//				float 			x  = Input.GetAxis( "RightH"  ) * cameraScript.sensitivity;	//If any of the values are 0, this becomes a noop, no need to check which input is used			
//				if(x == 0.0f) 	x  = Input.GetAxis( "Mouse X" ) * cameraScript.sensitivity;	//If any of the values are 0, this becomes a noop, no need to check which input is used			
//				
//				if (x != 0.0f)
//				{
//					//Rotate the character about the world Y axis.
//					thisTransform.RotateAround(Vector3.up, x * cameraScript.sensitivity * Time.deltaTime);
//				}
//			}
			...
			if(doJumpDown && canJump && !st.IsTag("Jump") && !st.IsTag("Land"))
			{
				...
				if(!st.IsTag("LedgeJump"))
					rb.velocity = character.up * jumpHeight;
			...
			{
				if(!doLShift)
					character.Rotate(0, mX * rotSpeed/2 * Time.deltaTime, 0);
			}
	...
	void WallGround ()
	{
		...
		bool wallHigh = Physics.Raycast (character.position + character.up*1.4f,
		                                 character.forward, col.radius*1.6f, wallRunLayers);
		
		// roughly the half of the characters height
		bool wallLow = Physics.Raycast (character.position + character.up,
		                                character.forward, col.radius*1.5f, wallRunLayers)
			&& !wallHigh;
	...
	void WallRun ()
	{
		...
		bool leftW = Physics.Raycast (character.position + character.up,
		                              character.right * -1 + character.forward/4, col.radius + 0.2f, wallRunLayers);
		bool rightW = Physics.Raycast (character.position + character.up,
		                               character.right + character.forward/4, col.radius + 0.2f, wallRunLayers);
		bool frontW = Physics.Raycast (character.position+ character.up,
		                               character.forward, col.radius + 0.2f, wallRunLayers);
		...
		if(!a.IsInTransition(0) && !rb.isKinematic)
		{
			if(leftW)
			{
				...
				rb.velocity = character.forward * Mathf.Abs(v) + character.up*3;
			...
			{
				rb.velocity = character.forward/2;
	...
	void CheckClimb ()
	{
		...
		if(doClimb && v > 0.2) // Up
		{
			...
			Vector3 p1 = character.position - (character.up * -heightOffsetToEdge) + character.forward;
			Vector3 p2 = character.position - (character.up * -heightOffsetToEdge) - character.forward;
			RaycastHit hit;
			
			// Hit nothing and not at edge -> Out
			if(!Physics.CapsuleCast (p1, p2, climbCheckRadius, character.up, out hit, climbCheckDistance, climbLayers))
				return;
			// If not almost facing the edge cancel/Out
			if (Vector3.Angle (character.right, hit.transform.right) >= 60.0f)
				return;
			...
			character.rotation = Quaternion.Euler(curT.eulerAngles.x, curT.eulerAngles.y, 0);
		...
		else if(doClimb && v < -0.2) // Down
		{
			...
			character.rotation = Quaternion.Euler(0,curT.eulerAngles.y,0);
			isTop = false; // RESET
	...
	void PullUp ()
	{
		if(st.IsTag("Top") && !a.IsInTransition(0))
		{
			...
			if (nT <= 1.0f)
			{
				if(nT <= 0.4f) // Step up
				{
					character.Translate(Vector3.up * Time.deltaTime * climbSpeed *8.0f);
				}
				else // Step forward
				{
					if(nT <= 0.6f)
						character.Translate(Vector3.forward * Time.deltaTime * climbSpeed *2.5f);
	...
	void EdgeClimb ()
	{
		Vector3 relPoint = curT.InverseTransformPoint (character.position);
		...
		if(!st.IsTag("Oh") && !a.IsInTransition(0)) // not if overhanging
		{
			...
			if(h != 0.0f) 
			{
				Vector3 origin = character.position - (character.forward * -climbOffsetToWall) - (character.up * -heightOffsetToEdge);
				if(h > 0.0f) // Check right
				{
					RaycastHit hit;
					if(Physics.SphereCast (origin, 0.2f, character.right, out hit, 0.4f, climbLayers))
					{
						nextPoint = hit.point;
						curT = hit.transform;
					...
					// Apply
					character.Translate( 1 * Time.deltaTime * edgeSpeed, 0, 0, curT );
				}
				else // Check left
				{
					RaycastHit hit;
	...
	void WallClimb ()
	{
		...
		Vector3 relV = curT.InverseTransformPoint(character.position);
		Vector3 inputVec = character.position;
		...
		if (distY >= heightOffsetToEdge * 1.1f)
		{
			...
			if (v != 0.0f)
			{
				if (v > 0.0f)
				{
					inputVec = character.TransformPoint( Vector3.up );
				} 
				else 
				{
					inputVec = character.TransformPoint( Vector3.down );
					
					if (!Physics.Raycast(character.position, character.forward, climbOffsetToWall, climbLayers) || grounded)
					{
						climbState = ClimbState.None;
					}
				}
				...
				// Apply movement
				character.position = Vector3.Lerp( character.position, inputVec, Time.deltaTime * climbSpeed * 1.3f);
			}
			else // Stop if there's no input
			{
				inputVec = character.TransformPoint( Vector3.zero );
			}
		}
	...
	void ClimbTo ()
	{
		...
		if(!st.IsTag("Pre"))
		{
			Vector3 edgePos = nextPoint + ((character.up * -heightOffsetToEdge) - (character.forward * climbOffsetToWall));
			// Apply lift up
			character.position = Vector3.Lerp (character.position, edgePos, Time.deltaTime * climbSpeed * 11);
			
			if(Vector3.Distance (character.position, edgePos) <=  0.1f)
			{
	...
	void CornerLerp (float speed)
	{
		Vector3 tVec = nextPoint - 
			(character.right * -cornerReach) - (character.forward * climbOffsetToWall) - (character.up * heightOffsetToEdge);
		
		float angleTo = Vector3.Angle (character.right, curT.transform.right);
		...
		// Apply
		character.position = Vector3.Lerp (character.position, tVec, Time.deltaTime * speed);
		character.rotation = Quaternion.Lerp (character.rotation, curT.rotation, Time.deltaTime * speed);
	...
	{		
		if (dist.ToString("f2") != desiredDist.ToString("f2")) 
		{
			...
			if (dist > desiredDist) // forward
			{
				...
				character.Translate( positive * s );
			}
			else if (dist < desiredDist) // backward
			{
				if (dist < (desiredDist + 0.1f))
					...
					s /= smoothDistanceSpeed /1.5f; // Near, move slower
				character.Translate( negative * s );
			}
		}
	}
	//=================================================================================================================o
	...
	void _Swim ()
	{
		float dTS = distY - character.position.y;
		...
		if(mX != 0.0f)
		{
			character.Rotate (0, mX * Time.fixedDeltaTime * diveRotSpeed * 55, 0, Space.Self);
		}
		...
		if(h != 0.0f || v != 0.0f) 
		{
			...
			{
				Vector3 swimInput = character.right * h + cam.transform.forward * v;	
				
				rb.velocity = swimInput.normalized * speed;
			}
			else if(grounded && dTS < offsetToSurf) // Walking in/out of the water
			{
				Vector3 swimInput = character.right * h + character.forward * v;	
				
				rb.velocity = swimInput.normalized * speed;
			}
			else // Above the surface
			{
				Vector3 swimInput = character.right * h + character.forward * v + liftVector;
	...
	{
		float targetAngle = Mathf.Abs (Vector3.Angle (cam.transform.forward, character.up));
	...
	{	
		...
		{
			// Movement Vector
			Vector3 flyVec = character.right * h 
	...
	void _Combat ()
	{
		...
		if(doCombat && canDrawHolster)
		{
			...
			else if(weaponState == WeaponState.Rifle)
			{
				...
				weaponSystem.currentGun.enabled = true;
		...
		if(grounded)
		{
			...
			//Rifle
			if(doAtk1 && weaponSystem.currentGun.freeToShoot || doAim){
			...
			firing = (firingTimer <= 0.0 && doAtk1);

						
			if (weaponSystem.currentGun != null)
			{
				//NGUIDebug.Log(weaponSystem.currentGun.fire);
...
//				}
	...
	void _PhysX ()
	{
		...
		character.position = Vector3.Lerp (character.position, pos, Time.deltaTime * 100.0f);
		...
		{
			Vector3 targetDir = rootBone.forward - character.up;
	...
	void StartRagdoll ()
	{
		...
		Vector3 vec = rb.velocity + character.forward *v*2 + character.right *h*2;
		...
		{
			if (bones[i].rigidbody && bones[i] != character.root)
			{
	...
	void EndRagdoll (int s)
	{
		...
		for (i=0; i<bones.Length; i++)
		{
			if (bones[i] != character.root && bones[i].rigidbody)
			{
	...
	void FeetPlacementIK () // Unity Pro only!
	{
		...
		if(LegLayerW > 0.1f)
		{
			RaycastHit rL;
			RaycastHit rR;
			Vector3 leftP = (character.position + character.up) - character.right/4.5f;
			Vector3 rightP = (character.position + character.up) + character.right/4.5f;
	...
	void AimLookIK () // Unity Pro only!
	{
		...
		{
			float delta = 1.0f - ((mainCamera.y + 85f) * 0.0058823529f);

			if(doAim && weaponState == WeaponState.Rifle)
...
//			
//			a.SetLookAtPosition (soldierTarget.position);
